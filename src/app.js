...
(function (global) {
  ...
  const ROTAS = [
    ...
    { hash: '#/playbook', ico: '🎯', nome: 'Playbook', render: V.playbook }
  ];
  ...
  function montarNav() {
    document.querySelector('nav.tabs').innerHTML = ROTAS.map(function (r) {
  ...
  const App = {
    ...
    novaEvidencia: function (opId) {
      ...
      U.formulario('Evidência do cliente', [
        { id: 'sugestao', rotulo: 'Evidência típica', tipo: 'select', opcoes: [{ valor: '', rotulo: '— descrever manualmente —' }].concat(sugestoes) },
        { id: 'titulo', rotulo: 'O que o cliente fez', placeholder: 'Ex.: CFO pediu o payback antes de aprovar' },
        ...
        if (!titulo) return;
        Store.registrarEvento(opId, { tipo: 'decision', titulo: titulo, dimensao: dimensao, canal: d.canal, data: d.data || Store.hoje() });
        render();
      });
    },
